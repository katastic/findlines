...
# Function to clean string of non-ASCII visible characters
clean_string() {
    local input="$1"
    # Use tr to keep only ASCII printable characters (32-126), replace others with space
    echo "$input" | tr -c '[:print:]\n' ' '
}
...
calculate_entropy() {
    ...
    for count in "${char_count[@]}"; do
        probability=$(bc <<< "scale=10; $count/$length")
        log_term=$(bc -l <<< "scale=10; l($probability)/l(2)")
        entropy=$(bc <<< "scale=10; $entropy - ($probability * $log_term)")
    done
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # Clean the line of non-ASCII characters before processing
    cleaned_line=$(clean_string "$line")
  #  echo "Original: $line"
  #  echo "Cleaned:  $cleaned_line"
    
    words=("$cleaned_line")
    
    for word in "${words[@]}"; do
        entropy=$(calculate_entropy "$word")
	# Skip analysis if word exceeds X characters
        if [ ${#word} -gt 40 ]; then
 #           echo "Skipped (too long, >40 chars): $word"
            continue
        fi        
